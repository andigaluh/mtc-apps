...
import equipment_excelService from "../services/equipment_excel.service"
...
const FormEquipmentExcel = () => {
    ...
    const createItem = (values, setSubmitting, resetForm) => {
        ...
        setTimeout(() => {
            ...
            equipment_excelService.create(data, (event) => {
    ...
    useEffect(() => {
        if (!isAddMode) {
            equipment_excelService.get(docId).then(
                (response) => {
                    const responseData = {
                        id: response.data.id,
            ...
            {!user.roles.includes("ROLE_SUPERVISOR") ? (
                ...
                <React.Fragment>
                    <div className={classes.titleContainer}>
                        ...
                        <Typography variant="h4" className={classes.title}>Equipment excel's Form</Typography>
                    </div>
                    <Card>
                        <CardContent>
                            <Formik
                                        ...
                                        <Grid container spacing={2}>
                                            <Grid item xs={12}>
                                                <Typography variant="body2">Please use this xls template file before upload : &nbsp;
                                                    <a href={`${process.env.REACT_APP_UPLOADS}equipment_template.xlsx`} target="_blank" className={classes.link}>
                                                        <Button variant="contained" className={classes.buttonContainer} >
                                                            <CloudDownload className={classes.button} /> equipment_template.xlsx
                                                        </Button>
                                                    </a>
                                                </Typography>
                                            </Grid>
...
export default FormEquipmentExcel;