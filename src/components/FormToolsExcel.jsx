...
import tools_excelService from "../services/tools_excel.service";
...
const FormToolsExcel = () => {
    ...
    const createItem = (values, setSubmitting, resetForm) => {
        ...
        setTimeout(() => {
            ...
            tools_excelService.create(data, (event) => {
    ...
    useEffect(() => {
        if (!isAddMode) {
            tools_excelService.get(docId).then(
                (response) => {
                    const responseData = {
                        id: response.data.id,
            ...
            {!user.roles.includes("ROLE_SUPERVISOR") ? (
                ...
                <React.Fragment>
                    <div className={classes.titleContainer}>
                        ...
                        <Typography variant="h4" className={classes.title}>Tools excel's Form</Typography>
                    </div>
                    <Card>
                        <CardContent>
                            <Formik
                                        ...
                                        <Grid container spacing={2}>
                                            <Grid item xs={12}>
                                                <Typography variant="body2">Please use this xls template file before upload : &nbsp;
                                                    <a href={`${process.env.REACT_APP_UPLOADS}tools_template.xlsx`} target="_blank" className={classes.link}>
                                                        <Button variant="contained" className={classes.buttonContainer} >
                                                            <CloudDownload className={classes.button} /> tools_template.xlsx
                                                        </Button>
                                                    </a>
                                                </Typography>
                                            </Grid>
